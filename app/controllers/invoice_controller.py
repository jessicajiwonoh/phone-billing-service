...
# Calculate rate per call per customer id
...
def calculate_calls_per_period(customer_id, month):
...
def get_monthly_invoice(customer_id):
    return Response(json.dumps(calculate_calls_per_period(customer_id, None)), mimetype='application/json')

@invoice_bp.route("/customer=<int:customer_id>/month=<int:month>")
def get_specific_month_invoice(customer_id, month):
    return Response(json.dumps(calculate_calls_per_period(customer_id, month)), mimetype='application/json')
...
def add_invoice_by_customer(customer_id):
    ...
    if today.day == 1:
        ...
        return Response(json.dumps(calculate_calls_per_period(customer_id, a_month_ago.month)), mimetype='application/json')
    else:
